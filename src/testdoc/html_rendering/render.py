...
class TestDocHtmlRendering():
    ...
    def render_testdoc(self,
        ...
        pyproject_toml = TOMLReader()._read_toml(str(Path(self.ROOT_DIR) / "pyproject.toml"))
        ...
        rendered_html = template.render(
            ...
            contact_mail = pyproject_toml.get('project').get('authors')[0].get('email'),
            tool_version = pyproject_toml.get('project').get('version')
        )
